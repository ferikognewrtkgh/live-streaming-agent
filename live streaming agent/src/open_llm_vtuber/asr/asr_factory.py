...
from .text_normalizer import ASRTextNormalizer
...
class ASRFactory:
    ...
    def get_asr_system(system_name: str, **kwargs) -> Type[ASRInterface]:
        if system_name == "faster_whisper":
            ...
            return ASRTextNormalizer(WhisperCPPASR(**kwargs))
        elif system_name == "whisper":
            from .openai_whisper_asr import VoiceRecognition as WhisperASR

            return ASRTextNormalizer(WhisperASR(**kwargs))
            ...
            return ASRTextNormalizer(SherpaOnnxASR(**kwargs))
        else:
