...
from ..emotion_tags import normalize_live2d_emotion_tag


def normalize_emotion_tag(value: object) -> str:
    return normalize_live2d_emotion_tag(value)
...
class Actions:
    ...
    motions: Optional[List[str] | List[dict]] = None
    ...
    def to_dict(self) -> dict:
        ...
        if self.emotions is not None:
            data["emotions"] = [
                normalized
                for normalized in (
                    normalize_emotion_tag(emotion) for emotion in self.emotions
                )
                if normalized
            ]
