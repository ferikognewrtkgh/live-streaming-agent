    ...
    def create_agent(
        ...
        **kwargs,
        ...
        if conversation_agent_choice == "basic_memory_agent":
            ...
            vision_llms = {}
            for vision_provider in (
                "doubao_vision_llm",
                "qwen3_vl_llm",
                "glm_5v_turbo_llm",
            ):
                vision_config = dict(llm_configs.get(vision_provider) or {})
                if not vision_config:
                    continue
                api_key = str(vision_config.get("llm_api_key") or "").strip()
                if not api_key or api_key.lower().startswith(("your ", "your_", "<")):
                    logger.info(
                        "Skipping unconfigured visual LLM provider: {}",
                        vision_provider,
                    )
                    continue
                vision_config.pop("interrupt_method", None)
                vision_llms[vision_provider] = StatelessLLMFactory.create_llm(
            ...
            return BasicMemoryAgent(
                ...
                scene_prompts=basic_memory_settings.get("scene_prompts", {}),
                vision_llms=vision_llms,
                default_vision_llm_provider=basic_memory_settings.get(
                    "vision_llm_provider"
                ),
            )
