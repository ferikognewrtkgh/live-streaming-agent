...
from .resource_paths import LIVE2D_MODELS_ROOT

DEFAULT_CLIENT_UID = "default"
...
def init_client_ws_route(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        client_uid = websocket.query_params.get("client_uid") or DEFAULT_CLIENT_UID
...
def init_webtool_routes(default_context_cache: ServiceContext) -> APIRouter:
    ...
    async def get_live2d_folder_info():
        ...
        for entry in live2d_dir.iterdir():
            if not entry.is_dir():
                continue

            folder_name = entry.name.replace("\\", "/")
            model3_path = entry / f"{folder_name}.model3.json"
            if not model3_path.is_file():
                continue

            # Keep URL payloads stable while scanning files from the frontend resource folder.
            model3_file = f"live2d-models/{folder_name}/{folder_name}.model3.json"
            avatar_file = None
            for ext in supported_extensions:
                avatar_path = entry / f"{folder_name}{ext}"
                if avatar_path.is_file():
                    avatar_file = f"live2d-models/{folder_name}/{folder_name}{ext}"
                    break
