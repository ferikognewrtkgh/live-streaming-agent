...
class KimiConfig(OpenAICompatibleConfig):
    """Configuration for Kimi through the Moonshot API."""

    base_url: str = Field("https://api.moonshot.cn/v1", alias="base_url")


class TencentYuanbaoConfig(OpenAICompatibleConfig):
    """Configuration for Tencent Yuanbao through TokenHub."""

    base_url: str = Field("https://tokenhub.tencentmaas.com/v1", alias="base_url")


class DoubaoVisionConfig(OpenAICompatibleConfig):
    """OpenAI-compatible visual model served by Volcano Engine Ark."""

    base_url: str = Field(
        "https://ark.cn-beijing.volces.com/api/v3", alias="base_url"
    )
    request_timeout_seconds: float = Field(60.0, alias="request_timeout_seconds")
    stream_idle_timeout_seconds: float = Field(
        30.0, alias="stream_idle_timeout_seconds"
    )


class Qwen3VLConfig(OpenAICompatibleConfig):
    """OpenAI-compatible Qwen3-VL model served by DashScope."""
...
class GLM5VTurboConfig(OpenAICompatibleConfig):
    """OpenAI-compatible GLM-5V-Turbo model served by BigModel."""

    base_url: str = Field(
        "https://open.bigmodel.cn/api/paas/v4/", alias="base_url"
    )
    request_timeout_seconds: float = Field(60.0, alias="request_timeout_seconds")
    stream_idle_timeout_seconds: float = Field(
        30.0, alias="stream_idle_timeout_seconds"
...
class StatelessLLMConfigs(I18nMixin, BaseModel):
    ...
    gemini_openai_llm: GeminiConfig | None = Field(
        None, alias="gemini_openai_llm"
    )
    zhipu_llm: ZhipuConfig | None = Field(None, alias="zhipu_llm")
    deepseek_llm: DeepseekConfig | None = Field(None, alias="deepseek_llm")
    kimi_llm: KimiConfig | None = Field(None, alias="kimi_llm")
    tencent_yuanbao_llm: TencentYuanbaoConfig | None = Field(
        None, alias="tencent_yuanbao_llm"
    )
    doubao_vision_llm: DoubaoVisionConfig | None = Field(
        None, alias="doubao_vision_llm"
    )
    qwen3_vl_llm: Qwen3VLConfig | None = Field(None, alias="qwen3_vl_llm")
    glm_5v_turbo_llm: GLM5VTurboConfig | None = Field(
        None, alias="glm_5v_turbo_llm"
    )
    ...
    DESCRIPTIONS: ClassVar[dict[str, Description]] = {
        ...
        "gemini_llm": Description(
            en="Configuration for Gemini API (native google-genai SDK)",
            zh="Gemini API 配置 (原生 google-genai SDK)",
        ),
        "gemini_openai_llm": Description(
            en="Configuration for Gemini API via OpenAI-compatible endpoint",
            zh="Gemini API 配置 (走 OpenAI 兼容端点)",
        ),
        ...
        "kimi_llm": Description(
            en="Configuration for Kimi through Moonshot API",
            zh="Kimi（Moonshot API）配置",
        ),
        "tencent_yuanbao_llm": Description(
            en="Configuration for Tencent Yuanbao through TokenHub",
            zh="腾讯元宝（TokenHub）配置",
        ),
        "doubao_vision_llm": Description(
            en="Configuration for Doubao Vision through Volcano Engine Ark",
            zh="豆包视觉模型（火山方舟）配置",
        ),
        "qwen3_vl_llm": Description(
            en="Configuration for Qwen3-VL through DashScope",
            zh="Qwen3-VL（阿里云百炼）配置",
        ),
        "glm_5v_turbo_llm": Description(
            en="Configuration for GLM-5V-Turbo through BigModel",
            zh="GLM-5V-Turbo（智谱开放平台）配置",
        ),
