...
class BasicMemoryAgentConfig(I18nMixin, BaseModel):
    ...
    scene_prompts: Optional[Dict[str, Any]] = Field(
        default_factory=dict, alias="scene_prompts"
    )
    vision_llm_provider: Optional[
        Literal[
            "doubao_vision_llm",
            "qwen3_vl_llm",
            "glm_5v_turbo_llm",
        ]
    ] = Field(None, alias="vision_llm_provider")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "scene_prompts": Description(
            en="Scene-based prompt injection config: append a scene-specific "
            "prompt after the resident persona prompt, chosen per-message by "
            "input source (e.g. barrage/mic).",
            zh="场景化提示词注入配置：在常驻人设提示词之后，按输入来源（如弹幕/麦克风）"
            "逐条追加对应的场景提示词。",
        ),
    }
