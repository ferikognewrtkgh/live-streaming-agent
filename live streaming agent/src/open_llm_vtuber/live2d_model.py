...
from .emotion_tags import extract_emotion_tags_from_text
from .resource_paths import MODEL_DICT_PATH
...
class Live2dModel:
    ...
    def __init__(
        self, live2d_model_name: str, model_dict_path: str | Path = MODEL_DICT_PATH
    ):
        self.model_dict_path: str = str(model_dict_path)
    ...
    def extract_emotion(self, str_to_check: str) -> list:
        """
        Check the input string for any emotion keywords and return a list of emotion tags.

        Parameters:
            str_to_check (str): The string to check for emotions.

        Returns:
            list: A list of emotion tags found in the string. An empty list is returned if no emotions are found.
        """

        return extract_emotion_tags_from_text(str_to_check, self.emo_map.keys())
