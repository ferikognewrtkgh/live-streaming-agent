...
class TTSInterface(metaclass=abc.ABCMeta):
    def supports_streaming_audio(self) -> bool:
        """Whether this TTS engine can produce incremental audio chunks."""
        return False
    ...
    async def async_generate_audio_stream(
        ...
        """
        Asynchronously generate speech audio chunks.

        Engines without native streaming fall back to a single complete audio file.
        """
