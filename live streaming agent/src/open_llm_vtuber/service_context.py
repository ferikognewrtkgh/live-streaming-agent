...
from .chat_history_manager import get_or_create_working_history
from .knowledge_elasticsearch import preload_knowledge_runtime
...
def _shorten_persona_prompt_for_log(character_config: CharacterConfig) -> CharacterConfig:
    persona_prompt = character_config.persona_prompt
    if len(persona_prompt) <= 20:
        return character_config
    return character_config.model_copy(
        update={"persona_prompt": f"{persona_prompt[:20]}..."}
    )
...
class ServiceContext:
    ...
    def __init__(self):
        ...
        self.knowledge_runtime: Any | None = None
        self.knowledge_turn_count: int = 0
        self.knowledge_last_injected_turn: int | None = None
        self.knowledge_last_injected_round_by_id: dict[str, int] = {}

    def _load_working_history(self) -> None:
        """Load the current working history and its latest summary into the agent."""
        if not self.character_config or not self.agent_engine:
            return

        history_uid = get_or_create_working_history(self.character_config.conf_uid)
        if not history_uid:
            return
        ...
        logger.info(
            f"Loaded working history {history_uid} for "
            f"{self.character_config.conf_uid}."
        )
    ...
    async def load_cache(
        ...
        self.knowledge_turn_count = 0
        self.knowledge_last_injected_turn = None
        self.knowledge_last_injected_round_by_id = {}
        ...
        logger.debug(
            ...
            _shorten_persona_prompt_for_log(character_config),
        )
        self._load_working_history()
    ...
    async def load_from_config(self, config: Config) -> None:
        ...
        self.init_knowledge(config.knowledge_config)
    ...
    def init_knowledge(self, knowledge_config: KnowledgeConfig) -> None:
        self.knowledge_runtime = None
        self.knowledge_turn_count = 0
        self.knowledge_last_injected_turn = None
        self.knowledge_last_injected_round_by_id = {}
        if not knowledge_config or not knowledge_config.enabled:
            logger.info("Knowledge base retrieval is disabled.")
            return

        try:
            self.knowledge_runtime = preload_knowledge_runtime(knowledge_config)
        except Exception as exc:
            logger.exception("Failed to initialize knowledge base retrieval.")
            logger.warning("Knowledge base retrieval disabled for this context: {}", exc)
            self.knowledge_runtime = None
    ...
    async def construct_system_prompt(self, persona_prompt: str) -> str:
        ...
        for prompt_name, prompt_file in self.system_config.tool_prompts.items():
            ...
            if prompt_name == "live2d_expression_prompt":
                logger.info(
                    "Skipping external live2d_expression_prompt; "
                    "emotion tag instructions are expected in conf.yaml persona_prompt."
                )
                continue
        ...
        try:
            ...
            if new_character_config_data:
                new_config = {
                    ...
                    "live_config": self.config.live_config.model_dump(),
                    "paint_config": self.config.paint_config.model_dump(),
                    "knowledge_config": self.config.knowledge_config.model_dump(),
                }
