...
SCENE_PROMPT_DIR = os.path.join(
    os.path.dirname(PROMPT_DIR),
    "resource",
    "prompt",
    "scenes",
)
...
def load_scene(scene_name: str) -> str:
    """Load the content of a specific scene prompt file."""
    scene_file_path = os.path.join(SCENE_PROMPT_DIR, f"{scene_name}.txt")
    try:
        return _load_file_content(scene_file_path)
    except Exception as e:
        logger.error(f"Error loading scene {scene_name}: {e}")
        raise
